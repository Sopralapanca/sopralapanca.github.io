...
async function setSliders(block, value, task){
	...
	for (var k=0; k<sliderBar.length; k++){
	...
	for (var k=0; k<hiddenField.length; k++){
		hiddenField[k].value = hiddenValue;
	}
	
	for (var k=0; k<SpeakerSimilarityField.length; k++){
...
var scope = document.getElementsByClassName("ewok-buds-query ewok-task-query")[0];

var requirements = document.getElementById("requirements-p");
var details = document.getElementsByClassName("details");

var body = document.getElementsByTagName("body")[0];
var editable = document.getElementById("editable-2");
var topTable = document.getElementsByTagName("table");
...
if (mode == "Search Product" && type == "Experimental"){
	var instructionsDiv = document.getElementsByClassName("ewok-buds-card");
...
/* EXP NOT AT ALL ARTICLE */
/* DA PROVARE SU UNA NUOVA NOT AT ALL DA SALVARE */

/*
if (mode == "Web" && type == "Experimental"){
	if (editable3Text != null && editable3Text.includes("In this task, you will be given the link to an article and a list of categories that may describe the content of the article. Your job is to evaluate whether the article’s content relates to any of the categories shown and if so, to rate the corresponding intensity level of issues related to that category as conveyed in the article.")){
		console.log("exp non at all found");
		
		var value = "25%";
		
		var sliderBar = document.getElementsByClassName("evl-slider2-bar-selected");
		var sliderTriangle = document.getElementsByClassName("evl-slider2-thumb evl-slider2-thumb-value goog-slider-thumb");
		
		var sliderDivider = document.getElementsByClassName("evl-slider2-thumb-divider");
		forse non serve lo slider divider
		
		for (var k=0; k<sliderBar.length; k++){
			sliderBar[k].style.width = value;
			sliderTriangle[k].style.left = value;
			
			mettere una funzione di sleep importante 
			setTimeout(function(){tmp.click()},2000+(Math.round(Math.random()*3000)));
			non va bene 
			await sleep(2000);
		}
		...
		var check = document.querySelector('input[name=isNoOtherDIsturbingOffensive]');
		setTimeout(function(){check.click()},2000+(Math.round(Math.random()*3000)));
		...
		if (editable14 == null){
			console.log("error exp not at all can't find link block");
			
		}
		
		var url = editable14.getElementsByTagName('a'), hrefs = [];
		for(var j=0; j<url.length; j++){
			console.log(url[j].href);
			var win = window.open(url[j].href, '_blank');
			win.focus();
		}
...
/* PSYCHO */
/* PROVATA E NON SCRIVE NELLA TEXTAREA RIVEDERE */

/* if (mode == "Mobile" && type == "Experimental"){
	if (time == "1.5 minute" || time == "2 minutes"){
		var editable5 = document.getElementById("editable-5");
		var editable5Text = null;
		if (editable5 != null){
			editable5Text = editable5.innerText;
		}
		if (editable5Text != null && editable5Text.includes("Comment (Required):")){
			console.log("psycho found");
			
			var comments = [
				"user intent was to know information about , the result provides correct and up to date information so it is very helpful for the user",
				"user was searching for , this result is helpful for the user because provides information related to the query",
				"user intent was to search for , the result shows helpful information so it is helpful for the user",
				"user intent was clear and was to search for , this result provides helpful information related to the query",
			]
			
			var commentBox = document.getElementsByName("comment");
			commentBox.value = comments[Math.floor(Math.random()*comments.length)];
			commentBox.innerHTML = comments[Math.floor(Math.random()*comments.length)];
		}
...
/* FORSE LE TOXIC DA 2.5 RIVEDERE 	
if(disclaimerText != null && disclaimerText.includes("In this task, you may be exposed to queries, webpages, and/or topics that contain potentially")){
	var radios = document.querySelectorAll('input[type=radio]');
	for(var i = 0; i< radios.length; i++){
		
		if(radios[i].value == "0" && !radios[i].name.includes("obscene_query1")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("insult_query1")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("threat_query1")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("identity_hate_query1")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("toxicity_level_query1")){
			radios[i].click();
		}
		
		
		if(radios[i].value == "0" && !radios[i].name.includes("obscene_query2")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("insult_query2")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("threat_query2")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("identity_hate_query2")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("toxicity_level_query2")){
			radios[i].click();
		}
		
		
		
		if(radios[i].value == "0" && !radios[i].name.includes("obscene_query3")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("insult_query3")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("threat_query3")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("identity_hate_query3")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("toxicity_level_query3")){
			radios[i].click();
		}
		
		
		if(radios[i].value == "0" && !radios[i].name.includes("obscene_query4")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("insult_query4")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("threat_query4")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("identity_hate_query4")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("toxicity_level_query4")){
			radios[i].click();
		}
		
		
		if(radios[i].value == "0" && !radios[i].name.includes("obscene_query5")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("insult_query5")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("threat_query5")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("identity_hate_query5")){
			radios[i].click();
		}
		if(radios[i].value == "0" && !radios[i].name.includes("toxicity_level_query5")){
			radios[i].click();
		}	
	}
